...
💉 = @. rand(
    Binomial(
        sample_sizes,
        (sero_sens * (1 - (sus_pop / NA_N))) + ((1-sero_spec) * (sus_pop / NA_N))
    )
)
...
ode_prior = sample(bayes_sir_tvp(K_window;
    ...
    sero_sample_sizes = sample_sizes[:,1:Window_size]
    ) | (y = Y[:,1:Window_size], z = 💉[:,1:Window_size]), Prior(), 1, discard_initial = 0, thinning = 1);
...
ode_nuts = sample(bayes_sir_tvp( K_window;
    ...
    obstimes = obstimes_window,
    sero_sample_sizes = sample_sizes[:,1:Window_size],
    )| (y = Y[:,1:Window_size], z = 💉[:,1:Window_size]), Turing.NUTS(1500, 0.65;), MCMCSerial(), 200, 1, discard_initial = 0, thinning = 1);
t2_init = time_ns()
runtime_init = convert(Int64, t2_init-t1_init)
plot(ode_nuts[240:250,:,:])
plot(loglikelihood(bayes_sir_tvp( K_window;
conv_mat = conv_mat_window,
knots = knots_window,
obstimes = obstimes_window,
sero_sample_sizes = sample_sizes[:,1:Window_size],
)| (y = Y[:,1:Window_size], z = 💉[:,1:Window_size]), ode_nuts)[150:250])
...
function generate_confint_infec_init(chn, y_data, z_data, K, conv_mat, knots, obstimes, sero_sample_sizes; cri = 0.95)
...
function generate_confint_recov_init(chn, y_data, z_data, K, conv_mat, knots, obstimes, sero_sample_sizes; cri = 0.95)
    chnm_res = generated_quantities(
        bayes_sir_tvp(K;
            conv_mat = conv_mat,
            knots = knots,
...
# Plot the infecteds
confint = generate_confint_infec_init(ode_nuts, Y[:,1:Window_size], 💉[:,1:Window_size], K_window, conv_mat_window, knots_window, obstimes_window, sample_sizes[:,1:Window_size])
...
# Plot the recovereds
confint = generate_confint_recov_init(ode_nuts, Y[:,1:Window_size], 💉[:,1:Window_size], K_window, conv_mat_window, knots_window, obstimes_window, sample_sizes[:,1:Window_size])
plot(confint.medci_inf', ribbon = (confint.medci_inf' - confint.lowci_inf', confint.uppci_inf' - confint.medci_inf')  , legend = false)
...
for idx_time_off_by_1 in eachindex(each_end_time[2:end])
    ...
    list_chains[idx_time] = sample(
        bayes_sir_tvp(K_window;
                ...
                knots = knots_window,
                obstimes = obstimes_window,
                sero_sample_sizes = sample_sizes[:,1:curr_t]
        ) | (y = y_data_window, z_data_window,),
