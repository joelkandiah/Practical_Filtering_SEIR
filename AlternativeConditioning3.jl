...
function sir_tvp_ode_no_win!(du, u, p_, t)
    ...
    @inbounds begin
        du[1,1] = -infection
        du[1,2] = infection - infectious
        du[1,3] = infectious - recovery
        du[1,4] = infection
        du[1,5] = infectious
    end
end;
...
prob_ode = ODEProblem(sir_tvp_ode_no_win!, Matrix(u0'), tspan, vcat(p, true_beta))
...
I_tot = Array(sol_ode(obstimes))[:,5,:] # Cumulative cases
...
@model function bayes_sir_tvp(
    ...
    function model_sir_tvp_ode!(du, u, p_, t)
        ...
        @inbounds begin
            du[1,1] = -infection
            du[2,1] = infection - infectious
            du[3,1] = infectious - recovery
            du[4,1] = infection
            du[5,1] = infectious
        end
    end;
