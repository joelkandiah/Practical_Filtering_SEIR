...
# u0[1,:,:] = NA_N - I0
# u0[4,:,:] = I0
const inv_gamma_prior = Gamma(1.43, 1/0.549)
const inv_γ = rand(inv_gamma_prior) + 2
const inv_σ = 3
const γ = 1/ inv_γ
const σ = 1/ inv_σ
# const ψ_dist = Gamma(20, 0.001)
const ψ_dist = truncated(Gamma(31.36,1/224); upper = 1000) 
...
const C = readdlm("ContactMats/england_8ag_contact_ldwk1_20221116_stable_household.txt") * 1e-6
...
a0_denom = map(k -> β_funcs[k](0) * inv_γ * abs(eigmax(diagm(NA_N[:,k]) * (sus_M .* C))), 1:NR)
a0 = a0_num ./ a0_denom
@tullio b0[i,k] := log(1 - (a0[k] * β_funcs[k](0) * sus_M[i,j] *  C[j,i])) |> (1 - exp((_)))
...
# a0_denom = map(k -> β_funcs[k](0) * inv_γ * real(eigmax(diagm(u0[1,:,k]) * (sus_M .* C))), 1:NR)
# 
# a0 = a0_num ./ a0_denom
...
IFR_vec = zeros(8)
IFR_vec[1:2] .= rand(Normal(-11.61184, 1.28185))
IFR_vec[3] = rand(Normal(-10.634830, 1.281734))
IFR_vec[4] = rand(Normal(-9.148093, 1.279731))
IFR_vec[5] = rand(Normal(-7.586303, 1.283084))
IFR_vec[6] = rand(Normal(-5.368160, 1.284837))
IFR_vec[7] = rand(Normal(-3.999493, 1.295454))
IFR_vec[8] = rand(Normal(-2.516182, 0.346383))
IFR_vec .= inverse(IFR_bijector).(IFR_vec)
...
const sero_sens = rand(Beta(71.5,29.5))
# sero_sens = 0.999
const sero_spec = rand(Beta(777.5,9.5))
# sero_spec = 0.999
...
#    βσ = βσ,
    ...
    ::Type{T_u0} = Float64,
    ::Type{T_Seir} = Float64;
) where {T_β <: Real, T_ψ <: Real, T_IFR <: Real, T_sus_M <: Real, T_u0 <: Real, T_Seir <: Real}
    ...
    η ~ truncated(Gamma(1,1/0.2), upper = minimum(N))

    # Set priors for ψ
    ψ = Vector{T_ψ}(undef, N_regions)
    ψ_EE  ~ truncated(Gamma(31.36,1/224); upper = 1000) 
    ψ_LDN ~ truncated(Gamma(31.36,1/224); upper = 1000) 
    ψ_MID ~ truncated(Gamma(31.36,1/224); upper = 1000)
    ψ_NEY ~ truncated(Gamma(31.36,1/224); upper = 1000) 
    ψ_NW  ~ truncated(Gamma(31.36,1/224); upper = 1000) 
    ψ_SE  ~ truncated(Gamma(31.36,1/224); upper = 1000)
    ψ_SW  ~ truncated(Gamma(31.36,1/224); upper = 1000) 
    ...
    # end
    # βₜ σ ~ Gamma(0.1,100)
    βₜσ ~ truncated(Gamma(1,1/100), upper = 3.0)
    β[1,1] = exp(log_β₀_EE)
    ...
    m_1 ~ truncated(Gamma(4,1/4), upper = 3.0)
    m_2 ~ truncated(Gamma(4,1/4), upper = 3.0)
    ...
    IFR_vec_init[1] ~ Normal(-11.61184, 1.28185)
    IFR_vec_init[2] ~ Normal(-10.634830, 1.281734)
    IFR_vec_init[3] ~ Normal(-9.148093, 1.279731)
    IFR_vec_init[4] ~ Normal(-7.586303, 1.283084)
    IFR_vec_init[5] ~ Normal(-5.368160, 1.284837)
    IFR_vec_init[6] ~ Normal(-3.999493, 1.295454)
    IFR_vec_init[7] ~ Normal(-2.516182, 0.346383)
    ...
    a0_denom = map(k -> β_functions[k](0) * inv_γ * real(eigmax(diagm(NA_N[:,k]) * (sus_M .* C))), 1:NR)
    a0 = a0_num ./ a0_denom

    @tullio b0[i,k] := log(1 - (a0[k] * β_functions[k](0) * sus_M[i,j] *  C[j,i])) |> (1 - exp((_)))
    tot_I₀ = inv_γ * map(k -> sum(b0[:,k] .* NA_N[:,k]) / a0_num[k], 1:NR)
    χ = map(k -> normalize(abs.(eigen(diagm(k) * sus_M .* C, sortby=x -> abs(x)).vectors[:,end]), 1), eachcol(NA_N)) |> stack
    tot_I₀_ages = χ .* transpose(tot_I₀)
...
const global_varnames = (@varname(η), @varname(m_1), @varname(m_2), @varname(sero_sens), @varname(sero_spec), @varname(βₜσ), @varname(d_I), @varname(IFR_vec_init))

const EE_varnames = (@varname(ψ_EE), @varname(log_β_EE))
const LDN_varnames = (@varname(ψ_LDN), @varname(log_β_LDN))
const MID_varnames = (@varname(ψ_MID), @varname(log_β_MID))
const NEY_varnames = (@varname(ψ_NEY), @varname(log_β_NEY))
const NW_varnames = (@varname(ψ_NW), @varname(log_β_NW))
const SE_varnames = (@varname(ψ_SE), @varname(log_β_SE))
const SW_varnames = (@varname(ψ_SW), @varname(log_β_SW))
