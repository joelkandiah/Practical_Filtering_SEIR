...
function sir_tvp_ode!(du::Array{T1}, u::Array{T2}, p_, t) where {T1 <: Real, T2 <: Real}
    ...
    # (_, infectious_1, infectious_2, recovery_1, recovery_2, b) = p_.cache_SIRderivs
    # (βt,) = p_.cache_beta
    # sus_M_tmp = t >= ld_start ? p_.sus_M[2,:,:,:] : p_.sus_M[1,:,:,:]
    p_.cache_C .= t <= ld_start ? p_.C_base : p_.C[Int(ceil(t / Δt_C)) - 5,:,:,:]
    ...
    p_.cache_SIRderivs[2] .= 2 .* σ .* E1
    p_.cache_SIRderivs[3]  .= 2 .* σ .* E2
    p_.cache_SIRderivs[4] .= 2 .* γ .* I1
    p_.cache_SIRderivs[5] .= 2 .* γ .* I2
    @inbounds begin
...
@model function bayes_sir_tvp(
    ...
    m_EE = Vector{T_sus_M}(undef, 11)
    m_LDN = Vector{T_sus_M}(undef, 11)
    m_MID = Vector{T_sus_M}(undef, 11)
    m_NEY = Vector{T_sus_M}(undef, 11)
    m_NW = Vector{T_sus_M}(undef, 11)
    m_SE = Vector{T_sus_M}(undef, 11)
    m_SW = Vector{T_sus_M}(undef, 11)
    ...
    sus_M[2,1,1:3,:] .= m_EE[7]
    sus_M[2,2,1:3,:] .= m_LDN[7]
    sus_M[2,3,1:3,:] .= m_MID[7]
    sus_M[2,4,1:3,:] .= m_NEY[7]
    sus_M[2,5,1:3,:] .= m_NW[7]
    sus_M[2,6,1:3,:] .= m_SE[7]
    sus_M[2,7,1:3,:] .= m_SW[7]
    ...
    sus_M[2,1,7,:] .= m_EE[10]
    sus_M[2,2,7,:] .= m_LDN[10]
    sus_M[2,3,7,:] .= m_MID[10]
    sus_M[2,4,7,:] .= m_NEY[10]
    sus_M[2,5,7,:] .= m_NW[10]
    sus_M[2,6,7,:] .= m_SE[10]
    sus_M[2,7,7,:] .= m_SW[10]
    
    sus_M[2,1,8,:] .= m_EE[11]
    sus_M[2,2,8,:] .= m_LDN[11]
    sus_M[2,3,8,:] .= m_MID[11]
    sus_M[2,4,8,:] .= m_NEY[11]
    sus_M[2,5,8,:] .= m_NW[11]
    sus_M[2,6,8,:] .= m_SE[11]
    sus_M[2,7,8,:] .= m_SW[11]
    ...
    Ĉ_base = stack(map(k -> sus_M[1,k,:,:] .* (C_base ./ eigmax_C_base), 1:NR), dims = 1)
    Ĉ = stack(map(j -> stack(map(k -> sus_M[2,k,:,:] .* (C[j,:,:] ./ eigmax_C_base), 1:NR), dims = 1), 1:size(C)[1]), dims = 1)
    ...
    display(tot_I₀)
    params_test = idd_params(p, N, β_functions, Ĉ_base, Ĉ, a0, cache_SIRderivs, cache_beta, cache_sus_M)
    ...
    for i in 1:length(obstimes)
        for j in 1:NR
            C_i = i <= ld_start ? Ĉ_base[j,:,:] : Ĉ[Int(ceil(i / Δt_C)) - 5, j,:,:]
            Rt[j,i] = a0[j] * β_functions[j](i) * inv_γ * real(eigmax(diagm(S[:,j,i]) * C_i[:,:]))
        end
    end

    return (; sol, y_μ, y, z, a0_num, β, β_functions, sus_M, Rt, prob_copy)
end;
...
conditions = (
    ...
    log_β_EE = post_params["log_beta_rw"]["param_value"][2:rw_length][1:K-1],
    log_β_LDN = post_params["log_beta_rw"]["param_value"][rw_length+2:2*rw_length][1:K-1],
    log_β_MID = post_params["log_beta_rw"]["param_value"][2*rw_length+2:3*rw_length][1:K-1],
    log_β_NEY = post_params["log_beta_rw"]["param_value"][3*rw_length+2:4*rw_length][1:K-1],
    log_β_NW = post_params["log_beta_rw"]["param_value"][4*rw_length+2:5*rw_length][1:K-1],
    log_β_SE = post_params["log_beta_rw"]["param_value"][5*rw_length+2:6*rw_length][1:K-1],
    log_β_SW = post_params["log_beta_rw"]["param_value"][6*rw_length+2:7*rw_length][1:K-1],
    sero_sens = post_params["sero_test_sensitivity"]["param_value"][1],  
    sero_spec = post_params["sero_test_specificity"]["param_value"][1], 
    d_I = post_params["infectious_period"]["param_value"],
    βₜσ = post_params["log_beta_rw_sd"]["param_value"][2],
    m_EE = post_params["contact_parameters"]["param_value"][2:12] .|> exp,
    m_LDN = post_params["contact_parameters"]["param_value"][12+2:2*12] .|> exp,
    m_MID = post_params["contact_parameters"]["param_value"][2*12+2:3*12] .|> exp,
    m_NEY = post_params["contact_parameters"]["param_value"][3*12+2:4*12] .|> exp,
    m_NW = post_params["contact_parameters"]["param_value"][4*12+2:5*12] .|> exp,
    m_SE = post_params["contact_parameters"]["param_value"][5*12+2:6*12] .|> exp,
    m_SW = post_params["contact_parameters"]["param_value"][6*12+2:end] .|> exp,
    log_λ₀_EE = post_params["log_p_lambda_0"]["param_value"][1],
    log_λ₀_LDN = post_params["log_p_lambda_0"]["param_value"][2],
    log_λ₀_MID = post_params["log_p_lambda_0"]["param_value"][3],
    ...
    IFR_vec_init = post_params["prop_case_to_hosp"]["param_value"][1:NA-1])
