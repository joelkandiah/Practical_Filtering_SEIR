...
gp = GP(0.2 * with_lengthscale(SqExponentialKernel(), 20))

f = gp(0.0:0.25:tmax, 0.01)
...
function sir_tvp_ode!(du::Array{T1}, u::Array{T2}, p_, t) where {T1 <: Real, T2 <: Real}
    ...
    b = p_.cache_b
    (infection, infectious, recovery) = p_.cache_SIR

    # βt = p_.β_function(t)
    @tullio grad=false threads=false b = p_.C[i,j] * I[i] * 0.22 / N 
    b *= p_.β_function[Int(floor(t / 0.25)) + 1]
    # println(b)
    infection .= b .* S
    infectious .= σ .* E
    recovery .= γ .* I
...
struct idd_params_with_cache_and_gps{T <: Real, T2 <: Real, T3 <: Real, T4 <: Real, T5 <: Real}
    params_floats::Vector{T}
    β_function::Vector{T2}
    N_regions::Int
    C::Matrix{T3}
    cache_b::T4
    cache_SIR::Vector{Vector{T5}}
end
...
params_test = idd_params_with_cache_and_gps(p, samp_f, NR, C, b, [infections, infectious, recovery])
...
@model function bayes_sir_tvp(
    ...
    α ~ truncated(Normal(0.0, 1.0), lower = 0.0)
    ...
    params_test = idd_params_with_cache(p, ConstantInterpolation(β, knots), 1, C, b, [infections, infectious, recovery]) 
    # Run model
    ...
    y_μ = (conv_mat * transpose(IFR_vec .* sol_X)) |>
        transpose
...
prob_window = remake(prob_ode,
    tspan = (0.0, Window_size)
)
