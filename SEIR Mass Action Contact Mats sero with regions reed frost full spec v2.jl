...
@model function bayes_sir_tvp(
    ...
    # Make by region and add lockdown
    ...
    tot_I₀ = inv_γ * map(k -> sum(b0[:,k] .* NA_N[:,k]) / a0_num[k], 1:N_regions)
    ...
    return (; sol, y_μ, y, z, β, β_functions)
end;
...
params_window = idd_params([0.0, 0.0],
    N_pop_R,
    map(x -> ConstantInterpolation(x, knots_window), eachcol(ones(K_window, NR))),
    C,
    ones(NA, NA),
    ones(NR),
...
init_model = bayes_sir_tvp(
        ...
        conditions = (IFR_vec_init = IFR_bijector.([0.0000078, 0.000017, 0.000038, 0.00028, 0.0041, 0.025, 0.12]), ψ_EE = 0.015, ψ_LDN = 0.015, ψ_MID = 0.015, ψ_SW = 0.015, ψ_NW = 0.015, ψ_SE  = 0.015, ψ_NEY = 0.015, d_I = 2.2, sero_spec = 0.99, βₜσ=0.1, sero_sens = 0.99) 
...
annoyingvarnames_4 = (@varname(m_1), @varname(m_2),)
annoyingvarnames_5 = (@varname(sero_sens), @varname(sero_spec),)
EE_varnames = (@varname(ψ_EE), @varname(log_β_EE))
LDN_varnames = (@varname(ψ_LDN), @varname(log_β_LDN))
MID_varnames = (@varname(ψ_MID), @varname(log_β_MID))
NEY_varnames = (@varname(ψ_NEY), @varname(log_β_NEY))
NW_varnames = (@varname(ψ_NW), @varname(log_β_NW))
SE_varnames = (@varname(ψ_SE), @varname(log_β_SE))
SW_varnames = (@varname(ψ_SW), @varname(log_β_SW))
...
global_regional_gibbs_mine = Gibbs(global_varnames => myamgs_η,
    ...
    annoyingvarnames_4 => myamgs_annoy4,
...
ode_nuts = sample(
    model_window | (sero_sens = 0.99, sero_spec = 0.99),
