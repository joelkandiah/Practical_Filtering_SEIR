# Imports 
# TODO: Move all other imports here
...
for seed_idx in eachindex(seeds_list)
    ...
    outdir = "seir sliding/10bp 1e6/Plot attempt $seed_idx/14_beta/120/"
    tmpstore = "PolsonExemplar/SIR_temporal_store/14_beta/120/"
...
# Define Gamma distribution by mean and standard deviation
    ...
    # chn_pri = sample(bayes_sir_tvp(Y, K), Prior(), 10)


    # cat(map(x ->Array(x.sol)[2,:], chnm_res)..., dims = 2)

    # (; sol, p) = bayes_sir_tvp(Y, K)() 
    ...
    ode_nuts = sample(bayes_sir_tvp_init(Y[1:Window_size], K_window;
        ...
        knots = knots_window,
        obstimes = obstimes_window,
        ), Turing.NUTS(1000, 0.65), MCMCThreads(), 1, n_chains, discard_initial = 150, thinning = 10)
